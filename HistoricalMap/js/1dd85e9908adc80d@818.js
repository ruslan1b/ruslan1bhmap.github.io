// https://observablehq.com/@michaeld/world-history-timeline@818
...
`
    ...
    main.variable(observer("getTooltipContent")).define("getTooltipContent", ["formatDate"], function(formatDate) {
        return (
            function(d) {
...
`
    ...
    main.variable(observer("dataByTimeline")).define("dataByTimeline", ["d3", "data"], function(d3, data) {
        return (
            d3.nest().key(d => d.timeline).entries(data)
        )
    });
    main.variable(observer("dataByRegion")).define("dataByRegion", ["d3", "data"], function(d3, data) {
        return (
            d3.nest().key(d => d.region).entries(data)
        )
    });
    main.variable(observer("axisBottom")).define("axisBottom", ["d3", "x", "formatDate"], function(d3, x, formatDate) {
        return (
            d3.axisBottom(x)
            .tickPadding(2)
    ...
    main.variable(observer("axisTop")).define("axisTop", ["d3", "x", "formatDate"], function(d3, x, formatDate) {
        return (
            d3.axisTop(x)
            .tickPadding(2)
            .tickFormat(formatDate)
    ...
    main.variable(observer("csv")).define("csv", ["d3"], function(d3) {
        return (
            d3.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vSZ2zYNVyNmsXD2mvSNgQBqdwCSouvey_3PFFFO29w2reMKK_cb5y8q9JZdaLxaQpxnKPMZ7Ddf93CP/pub?gid=999466116&single=true&output=csv")
        )
    });
    ...
    main.variable(observer("timelines")).define("timelines", ["dataByTimeline"], function(dataByTimeline) {
        return (
